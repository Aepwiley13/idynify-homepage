...
import { generateICPBrief } from './utils/generateICP.js';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'users', firebaseUser.uid), {
        icpData: formData,
        icpSubmitted: true,
        icpSubmittedAt: new Date().toISOString(),
        completedQuestions: 79
      });
      
      // 2. Generate ICP using Claude API
      console.log('Calling Barry AI to generate ICP...');
      const generatedICP = await generateICPBrief(formData, {
        company: user.company,
        email: user.email,
        name: user.name
      });
      
      console.log('ICP generated successfully:', generatedICP);
      
      // 3. Save generated ICP to Firebase
      await updateDoc(doc(db, 'users', firebaseUser.uid), {
        generatedICP: generatedICP,
        icpCompleted: true,
      ...
      setGenerationError(error.message || 'Failed to generate ICP. Please try again or contact support.');
      setGenerating(false);
    }
  };
  ...
  if (generating) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-pink-900 flex items-center justify-center p-4">
        <div className="max-w-2xl w-full bg-white/10 backdrop-blur-md rounded-2xl border border-purple-500/30 p-12 text-center">
          <div className="text-8xl mb-6 animate-bounce">🐻</div>
          <h1 className="text-4xl font-bold text-white mb-4">Barry AI is Analyzing Your Responses...</h1>
          <p className="text-xl text-purple-200 mb-8">Creating your custom ICP brief (30-60 seconds)</p>
          
          <div className="bg-white/20 rounded-full h-3 mb-8 overflow-hidden">
            <div className="bg-gradient-to-r from-cyan-400 to-pink-400 h-full animate-pulse" style={{width: '70%'}}></div>
          </div>
          
          <div className="space-y-3 text-purple-300">
            <p>✅ Processing 79 questionnaire responses</p>
            <p>✅ Analyzing your ideal customer profile</p>
            <p>⏳ Generating 8-section ICP brief...</p>
          </div>
          
          {generationError && (
            <div className="mt-8 bg-red-500/20 border border-red-500/30 rounded-xl p-6">
              <p className="text-red-300 font-semibold mb-2">❌ Generation Error</p>
              ...
              <button
                ...
                className="mt-4 bg-white/20 hover:bg-white/30 text-white px-6 py-2 rounded-lg"
              >
                Try Again
              </button>
            </div>
          ...
          <div className="flex justify-between items-center mt-8">
            ...
            <div className="flex items-center gap-4">
              ...
              {isLastQuestion() ? (
                ...
                >
                  {generating ? 'Generating ICP...' : 'Complete & Generate ICP'}
                </button>
              ) : (
                <button
                  onClick={handleNext}
